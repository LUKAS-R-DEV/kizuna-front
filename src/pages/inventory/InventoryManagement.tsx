        ...
        <div className="flex flex-col gap-6 border-l-[6px] border-red-700 pl-5 py-4 bg-white/40 backdrop-blur-sm">
              ...
              <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
                ...
                <DialogContent className="sm:max-w-[500px] bg-white border-t-[6px] border-red-700">
                  <DialogHeader>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Master Data Input // Kizuna Industrial</p>
                  </DialogHeader>
          ...
          <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit">
            <button
              ...
              ${location.pathname === "/inventory" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all
              ${location.pathname === "/inventory/movements" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHead className="text-[9px] font-black uppercase pl-6 h-12">Material ID</TableHead>
                <TableHead className="text-[9px] font-black uppercase">Material Specification</TableHead>
                <TableHead className="text-[9px] font-black uppercase text-center">Category</TableHead>
                <TableHead className="text-[9px] font-black uppercase text-center">Stock Level</TableHead>
                <TableHead className="text-[9px] font-black uppercase text-center">Status</TableHead>
                <TableHead className="text-[9px] font-black uppercase pr-6 text-right">Location</TableHead>
              </TableRow>
            </TableHeader>

            <TableBody>
              {INVENTORY_MOCK.map((item) => (
                <TableRow key={item.id} className="group hover:bg-slate-50 transition-colors border-b border-slate-50/50">
                  <TableCell className="pl-6 font-black text-slate-400 italic text-[10px]">
                    {item.id}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-center">
                    <Badge variant="outline" className="text-[8px] font-black uppercase bg-slate-50 text-slate-500">
                      {item.category}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[8px] font-bold text-slate-300 uppercase">Min: {item.minStock}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <Badge
                  ...
                  <TableCell className="text-right pr-6 font-bold italic text-[10px] text-slate-500 uppercase">
                    {item.location}
                  </TableCell>
                </TableRow>
              ))}
