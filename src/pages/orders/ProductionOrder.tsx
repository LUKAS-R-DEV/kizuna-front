          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            ...
            <DialogContent className="max-w-md border-t-4 border-red-600">
              <DialogHeader>
                ...
                <DialogDescription className="text-[10px] font-bold text-slate-800 uppercase tracking-widest">Register Operational Task</DialogDescription>
              </DialogHeader>
              
              <form onSubmit={handleCreateOrder} className="space-y-4 py-2 italic font-medium">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-800 uppercase ml-1">Product Description</label>
                  <Input 
                    required 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-800 uppercase ml-1">Quantity</label>
                    <Input 
                      required 
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-800 uppercase ml-1">Priority</label>
                    <select 
            ...
            <TableBody>
              {filteredOrders.map((order) => (
                <TableRow key={order.id}>
                  <TableCell>
                    <span className="font-black text-slate-900 italic">{order.id}</span>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2 font-bold text-slate-700">
                      <ClipboardList size={14} className="text-slate-800" />
                      {order.product}
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className="text-slate-700 font-medium text-sm">{order.quantity}</span>
                  </TableCell>
        ...
        <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
          <DialogContent className="max-w-md border-t-4 border-blue-600">
            <DialogHeader>
              ...
              <DialogDescription className="text-[10px] font-bold text-slate-800 uppercase tracking-widest underline underline-offset-4">Technical Specifications</DialogDescription>
            </DialogHeader>
            ...
            {selectedOrder && (
              <div className="py-4 space-y-6">
                <div className="grid grid-cols-2 gap-6 bg-slate-50 p-4 rounded-lg border border-slate-100">
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-800 uppercase">Product</span>
                    <p className="font-bold text-slate-800 tracking-tight leading-tight">{selectedOrder.product}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-800 uppercase">Quantity</span>
                    <p className="font-bold text-slate-800 tracking-tight leading-tight">{selectedOrder.quantity}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-800 uppercase">Deadline</span>
                    <div className="flex items-center gap-1.5 font-bold text-slate-800 italic">
                      <Calendar size={12} /> {selectedOrder.deadline}
                    </div>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-800 uppercase">Assigned Operator</span>
                ...
                <div className="flex justify-between items-center border-l-2 border-slate-200 pl-4">
                  <div className="space-y-1">
                    <span className="text-[9px] font-black text-slate-800 uppercase">Current Process Status</span>
