        ...
        <div className="flex items-end justify-between border-l-[6px] border-red-700 pl-5 py-4 bg-white/40 backdrop-blur-sm">
          <div className="flex flex-col text-left">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-3xl font-thin text-slate-300 tracking-tighter uppercase">Inspection</span>
            </div>
          </div>
                  ...
                  <div key={batch.id} className={`p-5 rounded-xl transition-all border ${batch.status === 'Pending' ? 'border-2 border-blue-600 shadow-md bg-white' : 'border-slate-100 bg-slate-50'}`}>
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <span className={`text-[10px] font-black italic ${batch.status === 'Pending' ? 'text-blue-600' : 'text-slate-400'}`}>{batch.id}</span>
                        <h3 className={`text-lg font-black uppercase leading-none ${batch.status === 'Pending' ? 'text-slate-900' : 'text-slate-400'}`}>{batch.productName}</h3>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                          PO: {batch.poNumber} <span className="mx-2">|</span> QTY: {batch.quantity} UNITS
                        </p>
                      </div>
                      <Badge className={`${batch.status === 'Pending' ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-500'} border-none font-black text-[9px] px-3 uppercase`}>
                        {batch.status}
                      </Badge>
                    </div>
                  </div>
                ...
                {MOCK_QUALITY_HISTORY.map((item) => (
                  <div key={item.id} className="p-5 flex items-start justify-between">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-slate-500 bg-slate-50 p-2 rounded border border-slate-100">
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-lg overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1 tracking-widest">Current Selection</p>
                  <p className="text-xl font-black text-slate-900 tracking-tighter italic leading-none">{MOCK_QUALITY_BATCHES[0].id}</p>
                  <p className="text-xs font-bold text-slate-600 uppercase mt-1">{MOCK_QUALITY_BATCHES[0].productName}</p>
                </div>
